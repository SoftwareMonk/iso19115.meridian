...
xmlns:gmx="http://www.isotc211.org/2005/gmx" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	...
	<xsl:variable name="metadataProcessorEmail" select="'k.mortimer@dal.ca'"/>
	...
	<xsl:template match="gmi:MI_Metadata">
		 <xsl:copy>
			...
			<xsl:if test="not(@gco:isoType)">
				<xsl:attribute name="gco:isoType">gmi:MI_Metadata</xsl:attribute>
			</xsl:if>
		 	<xsl:choose>
		...
		<!-- **should this be describes, or description? -->
	...
	<gmd:CI_ResponsibleParty>
			<gmd:individualName>
				<gco:CharacterString><xsl:value-of select="$metadataProcessorName"/></gco:CharacterString>
			</gmd:individualName>
			<gmd:organisationName>
				<gco:CharacterString>MERIDIAN - Marine Environmental Research Infrastructure for Data Integration and Application Network</gco:CharacterString>
			</gmd:organisationName>
			<gmd:contactInfo>
				<gmd:CI_Contact>
					<gmd:address>
						<gmd:CI_Address>
							<gmd:deliveryPoint>
								<gco:CharacterString>6050 University Avenue</gco:CharacterString>
							</gmd:deliveryPoint>
							<gmd:city>
								<gco:CharacterString>Halifax</gco:CharacterString>
							</gmd:city>
							<gmd:administrativeArea>
								<gco:CharacterString>N.S.</gco:CharacterString>
							</gmd:administrativeArea>
							<gmd:postalCode>
								<gco:CharacterString>B3H 4R2</gco:CharacterString>
							</gmd:postalCode>
							<gmd:country>
								<gco:CharacterString>Canada</gco:CharacterString>
							</gmd:country>
							<gmd:electronicMailAddress>
								<gco:CharacterString><xsl:value-of select="$metadataProcessorEmail"/></gco:CharacterString>
							</gmd:electronicMailAddress>
						</gmd:CI_Address>
					</gmd:address>
				</gmd:CI_Contact>
	...
	<gmd:MD_MaintenanceInformation>
			...
			<gmd:contact>
				<gmd:CI_ResponsibleParty>
					...
					<gmd:role>
						<gmd:CI_RoleCode codeList="gmxCodeLists-MERIDIAN.xml#CI_RoleCode" codeListValue="processor">processor</gmd:CI_RoleCode>
					</gmd:role>
				</gmd:CI_ResponsibleParty>
			</gmd:contact>
	</gmd:MD_MaintenanceInformation>
	...
	<xsl:template match="*[@codeListValue]">
		<xsl:copy>
			...
			<xsl:attribute name="codeList">
				<xsl:value-of select="concat('**FIXME URL OF OUR CODELISTS',local-name(.))"/>
			</xsl:attribute>
	...
	<!-- TODO: does this need to be done for every possible namespace? -->
	<!-- ================================================================= -->
	...
	<xsl:template match="gmi:*">
		<xsl:call-template name="correct_ns_prefix">
			...
			<xsl:with-param name="prefix" select="'gmi'"/>
		</xsl:call-template>
	</xsl:template>
